...
#define ACCELERATION 3.36
#define DECELERATION 5.59
...
void turn90Degrees() {
    ...
    int turnSpeed = 0;
    while (turnSpeed < targetSpeed) {
        turnSpeed += accelerationStep;
        if (turnSpeed > targetSpeed) {
    ...
    while (leftEncoderCount < countsPer90DegreeTurn && rightEncoderCount < countsPer90DegreeTurn);
...
void PID_travel(uint8_t sensorValue) {
...
int main(void) {
    ...
    DDRB &= ~((1 << BUTTON_PIN_INCREMENT) | (1 << BUTTON_PIN_DECREMENT) | (1 << BUTTON_PIN_START)) | (1 << BUTTON_PIN_SERVED);
    // Enable pull-up resistors
    PORTB |= (1 << BUTTON_PIN_INCREMENT) | (1 << BUTTON_PIN_DECREMENT) | (1 << BUTTON_PIN_START) | (1 << BUTTON_PIN_SERVED);
    ...
    while (1) {
        ...
        float leftDistance = calculate_distance_traveled(leftEncoderCount);
        float rightDistance = calculate_distance_traveled(rightEncoderCount);
        ...
        if(start){
            ...
            if (serving_state == 2){
                ...
                while(true){
                    if (debounce_button(BUTTON_PIN_SERVED)) {
                        serving_state = 3; // Start the robot
                        mode = 2; // Set mode to accelerating
                        // Wait until button is released
                        while (debounce_button(BUTTON_PIN_SERVED));
                    }
                }
                // Move the robot back to the starting point
                turn90Degrees();    // Turn 90 degrees back
                ...
                if (table_count == 0) {
                    turn90Degrees();    // Turn 90 degrees back
                    turn90Degrees();    // Turn 90 degrees back
                    serve_state = 1; // Set the serving state to none
                    start = false; // Set the start to false
